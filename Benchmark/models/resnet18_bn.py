...
class ResNet_mnist_bn(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_planes=64):
        super(ResNet_mnist, self).__init__()
        self.in_planes = in_planes

        self.conv1 = nn.Conv2d(1, in_planes, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.layer1 = self._make_layer(block, in_planes, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, in_planes * 2, num_blocks[1], stride=2)
...
def ResNet18_bn(in_planes=64):
    return ResNet(BasicBlock, [2, 2, 2, 2], in_planes=in_planes)

def ResNet18_mnist_bn(in_planes=64):
    return ResNet_mnist(BasicBlock, [2, 2, 2, 2], in_planes=in_planes)

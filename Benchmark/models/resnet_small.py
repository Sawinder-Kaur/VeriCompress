...
class ResNet5(nn.Module):
    def __init__(self, block, in_ch = 1, num_blocks=2, num_classes=10, in_planes=64, bn=True, last_layer="avg"):
        super(ResNet5, self).__init__()
        self.in_planes = in_planes
        self.bn = bn
        self.last_layer = last_layer
        self.conv1 = nn.Conv2d(in_ch, in_planes, kernel_size=3,
        ...
        if self.last_layer == "avg":
            ...
            self.linear1 = nn.Linear(1024,100) # use 784,100 for mnist
...
class ResNet5_mnist(nn.Module):
    def __init__(self, block, in_ch = 1, num_blocks=2, num_classes=10, in_planes=64, bn=True, last_layer="avg"):
        super(ResNet5_mnist, self).__init__()
        self.in_planes = in_planes
        self.bn = bn
        self.last_layer = last_layer
        self.conv1 = nn.Conv2d(in_ch, in_planes, kernel_size=3,
        ...
        if self.last_layer == "avg":
            ...
            self.linear1 = nn.Linear(in_planes * 8 * block.expansion * 16, 100)
            self.linear1 = nn.Linear(784,100) # use 784,100 for mnist
            self.linear2 = nn.Linear(100, num_classes)
        else:
            exit("last_layer type not supported!")
...
def resnet5_svhn():
    return ResNet5_svhn(BasicBlock, num_blocks=2, in_planes=8, bn=False, last_layer="dense")

def resnet5():
    return ResNet5(BasicBlock,in_ch = 3, num_blocks=2, in_planes=8, bn=False, last_layer="dense")

def resnet9():
    return ResNet9(BasicBlock,in_ch = 3, num_blocks=2, in_planes=16, bn=False, last_layer="dense")

def resnet5_mnist():
    return ResNet5_mnist(BasicBlock,in_ch = 1, num_blocks=2, in_planes=8, bn=False, last_layer="dense")

def resnet9_mnist():
    return ResNet9(BasicBlock,in_ch = 1, num_blocks=2, in_planes=16, bn=False, last_layer="dense")

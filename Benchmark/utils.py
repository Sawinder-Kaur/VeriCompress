...
def apply_mask(model, model_mask,struct_mask = None):
    ...
    for module in model.modules():
        ...
        if isinstance(module, nn.Conv2d) or isinstance(module, nn.Conv1d) or isinstance(module, nn.Linear):
            ...
            if len(model.state_dict()[layer_names[b_ln]]) != 10 and struct_mask != None: 
...
def remove_redundant_nodes(model,example_inputs,parameter_budget):
    ...
    for m in model.modules():
        if isinstance(m, torch.nn.Conv2d)or isinstance(m, torch.nn.BatchNorm2d) or (isinstance(m, torch.nn.Linear) and m.out_features == 10):
...
def deactivate(model, args):
    if args.data == 'MNIST':
        ...
        model_mask = models.Models[args.model](in_ch=1, in_dim=28)
    else:
        dummy_input = torch.randn(1, 3, 32, 32)
        if args.model == 'vgg16':
            model_mask = models.Models[args.model]()
        else :
            model_mask = models.Models[args.model](in_ch=3, in_dim=32)

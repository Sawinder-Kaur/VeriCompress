...
class cnn_4layer(nn.Module):
    def __init__(self, in_ch, in_dim, width=4, linear_size=100):
...
def cnn_16layer(in_ch, in_dim, width=64, linear_size=256):
    model = nn.Sequential(
        nn.Conv2d(in_ch, width, 3, stride=1, padding=1),
        nn.ReLU(),
        nn.Conv2d(width, width, 3, stride=1, padding=1),
        ...
        nn.ReLU(),
        nn.Conv2d(4 * width, 4 * width, 3, stride=2, padding=1),
        nn.ReLU(),
        nn.Conv2d(4 * width, 4 * width, 3, stride=1, padding=1),
        nn.ReLU(),
        nn.Conv2d(4 * width, 4 * width, 3, stride=1, padding=1),
        nn.ReLU(),
        Flatten(),
        nn.Linear(4 * width * 2 * 2, linear_size),
        nn.ReLU(),

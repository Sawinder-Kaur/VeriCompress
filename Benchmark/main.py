...
parser.add_argument("--data", type=str, default="CIFAR", choices=["MNIST", "CIFAR","SVHN"], help='dataset')
...
def main(args):
    ...
    if args.data == 'MNIST':
        ...
        train_data.mean = test_data.mean = torch.tensor([0.5, 0.5, 0.5])
        train_data.std = test_data.std = torch.tensor([0.5, 0.5, 0.5])
    ...
    if args.compress == "True": 
        prev_state = deactivate(model_ori, args)
    ...
    if args.verify:
        ...
        for t in range(epoch + 1, args.num_epochs+1):
            ...
            if args.compress == "True" and t% args.thickening_length == 0: 
                new_state = deactivate(model_ori, args)
                #print(element_state)
                state_diff = 0
                for ns,ps in zip(new_state,prev_state):
                    state_diff += torch.sum(torch.abs(ns - ps)).item()
                print("Difference in states : {}".format(state_diff))
        ...
        if args.deploy == "True": 
            ...
            remove_redundant_nodes(model_ori,dummy_input,parameter_budget)
